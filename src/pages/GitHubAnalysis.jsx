...
const TEST_PROFILES = [
  ...
  { id: 'full', name: 'Full Audit', desc: 'All 19 modules', modules: ['failureMode', 'security', 'hallucination', 'oracle', 'complexity', 'mutation', 'property', 'differential', 'typescript', 'accessibility', 'dependency', 'deadCode', 'raceCondition', 'apiContract', 'errorHandling', 'regexAnalysis', 'logging', 'customRules', 'aiReview'], icon: Search, color: 'text-blue-600 bg-blue-50 border-blue-200' },
]
...
export default function GitHubAnalysis() {
  ...
  const handleAnalyze = async (data) => {
    ...
    try {
      ...
      const crit = actionable.filter(f => f.severity === 'Critical').length
      const hi = actionable.filter(f => f.severity === 'High').length
      const med = actionable.filter(f => f.severity === 'Medium').length
      const weighted = (crit * 10) + (hi * 5) + (med * 2)
